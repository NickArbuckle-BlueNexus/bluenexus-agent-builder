...
Set a response constraint (the screening prompt) **and** put the core scope in the **Rules field** ("You only help with {domain}; politely refuse anything else"). The Rules field is model-level and the most dependable place for scope, so treat it as your primary guarantee and set it every time. **Verify:** send an off-scope probe in Test chat, and again on the published surface if it matters — Test chat and the live bot can behave differently.
